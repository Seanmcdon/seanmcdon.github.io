function Exercise () {
    ...
    self.displayAnswerChoices = function() {
        ...
        //push the answer on 
        var answerEl = $(".interval-toggle[name='"+self.answer+"']")[0];
        randomizedArr.push(answerEl);
        
        //how many choices?
        var selected = document.getElementById("select-amount").value;
                  
        console.log(selected);
        
        //create a random node
        var randomEl = self.chooseToggleButton();
        var randomNote = $(randomEl).attr("name");
        
        while(randomizedArr.length < selected){
            
            var found = randomizedArr.find(element => $(element).attr("name") == randomNote);        
            console.log(found);
            
            if(found){
                randomEl = self.chooseToggleButton();
                randomNote = $(randomEl).attr("name");
            }
            
            else{ 
            
                randomizedArr.push(randomEl);

            }    
            
        }
        
        shuffle(randomizedArr);
        
        for (var i = 0; i < randomizedArr.length; ++i) {
            var choice = $(randomizedArr[i]).attr('answer');
            var choice_name = $(randomizedArr[i]).html();
            var hotkey = $(randomizedArr[i]).attr('hotkey');
